        ...
        user.find({status: "student"}).count(function (err, stuCount) {
            ...
            user.find({status: "instructor"}).count(function (err, insCount) {
                ...
                group.find().count(function (err, groupCount) {
                    ...
                    res.render('index', {
                        countStudent: totalNumberStudent,
                        countInstructor: totalNumberInstructor,
                        countGroup: totalNumberTeam
                    });
                });
            });
        });
...
app.post("/inst_profile_check", function (req, res) {
    ...
    message.find({}, function (err, messages) {
        res.render("instructor-profile", {
            passwordCheck: passwordFlag,
            changed: changeSuccess,
            fullName: fullName,
            email: userEmail,
            messages: messages
        });
        ...
        user.find({}, function (err, result) {
            ...
            for (var i = 0; i < result.length; i++) {
                if (result[i].email == leaderEmailAddr) {
                    memberList.push({
                        givenName: result[i].givenName,
                        familyName: result[i].familyName,
                        emailAddr: result[i].email
                    });
                }
            }
                ...
                for (var j = 0; j < members.length; j++) {
                    for (var k = 0; k < result.length; k++) {
                        if (result[k].email == members[j].studentEmail) {
                            memberList.push({
                                givenName: result[k].givenName,
                                familyName: result[k].familyName,
                                emailAddr: result[k].email
                            });
                        }
                    }
                }
